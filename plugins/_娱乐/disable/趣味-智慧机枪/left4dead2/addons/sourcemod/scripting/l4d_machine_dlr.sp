...
public OnPluginStart()
{
	...
	//l4d_machine_damage_of_tank=CreateConVar("l4d_machine_damage_of_tank", "10.0", "bullet damage for tank", FCVAR_PLUGIN);
	...
	l4d_machine_fire_rate=CreateConVar("l4d_machine_fire_rate", "20", "rate of fire, how many shot per soncods [5, 30]", FCVAR_PLUGIN);
 	...
 	l4d_machine_betray_chance=CreateConVar("l4d_machine_betray_chance", "10.0", "betray chance", FCVAR_PLUGIN); 	
	...
	RegConsoleCmd("sm_dlrpmkai", sm_machine);  
	RegConsoleCmd("sm_dlrpmkairm", sm_removemachine);  
	ResetAllState();
	GetConVar();
...
SpawnMiniGun(client , index ,type)
{
	...
	SetEntData( gun, g_CollisionOffset, 1, 4, true );
...
StopCarry(index)
{
	if(GunCarrier[index]>0)
	{
		...
		//SetEntProp(Gun[index], Prop_Send, "m_CollisionGroup", 0);  
	 	SetEntData(Gun[index], g_CollisionOffset, 1, 4, true );
